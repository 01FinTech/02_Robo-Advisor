...
def init(context):
    ...
    context.stocks = []
    context.stocks.append( "000001.XSHE" )
    context.stocks.append( "601998.XSHG" )
    context.stocks.append( "000068.XSHE" )
    ...
    '''
    这个函数可以让你查询到每日、每周或每月的某个指标的雪球数据的股票排名情况以及它的对应的统计数值，不传入date参数的话默认就是拿到回测到当天的前一天的统计数据（当天只能拿到前一天的数据）

注意：我们最早支持的雪球数据只到2015年4月23日，之后的数据我们都会保持更新。
'''
